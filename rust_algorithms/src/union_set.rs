...
fn merge(set: &mut UnionSet, first: i32, second: i32) {
	...
	set.subtree_size[parent_first] = set.subtree_size[parent_first] + set.subtree_size[parent_second];
	set.parent_index[parent_second] = first;
    } else {
	set.subtree_size[parent_second] = set.subtree_size[parent_second] + set.subtree_size[parent_first];
	set.parent_index[parent_first] = second;
    }
}

fn find(set: &UnionSet, index: i32) -> i32 {
    let mut parent: i32 = set.parent_index[index as usize];
    let mut result = index;
    println!("parent: {parent}, result: {result}");
    while parent != -1 {
	result = parent;
...
mod tests {
    ...
    fn test_merge()
    {
	let mut example = UnionSet {
	    parent_index: vec![-1, -1, -1, -1, -1, -1, -1],
	    subtree_size: vec![1, 1, 1, 1, 1, 1, 1],
	    count: 7
	};
